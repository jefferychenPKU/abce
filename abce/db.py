...
class Database(Process):
    ...
    def run(self):
        ...
        while True:
            ...
            if typ == 'panel':
                ...
                round = int(in_sok.recv()) #pylint: disable=W0622
    ...
    def write_or_update(self, table_name, data_to_write):
        insert_str = "INSERT OR IGNORE INTO %s (%s) VALUES (%s);" % (
            table_name,
            ','.join(data_to_write.keys()),
            ','.join(["%f"] * len(data_to_write)),
        )
        update_str = "UPDATE %s SET %s  WHERE CHANGES()=0 and round=%s and id=%s;" % (
                table_name,
                ','.join('%s=%s' % (key, data_to_write[key]) for key  in data_to_write),
                data_to_write['round'],
                data_to_write['id']
        )
        rows_to_write = data_to_write.values()
        try:
            self.database.execute(insert_str % tuple(rows_to_write))
        except (sqlite3.OperationalError, sqlite3.SQLException), msg:
            ...
            self.write_or_update(table_name, data_to_write)
        self.database.execute(update_str)

    def write(self, table_name, data_to_write, ex_str=''):
        ex_str = "INSERT INTO " + table_name + "(" + ','.join(data_to_write.keys()) + ") VALUES (%s)" % ','.join(["%f"] * len(data_to_write))
        rows_to_write = data_to_write.values()
        format_strings = ','.join(['?'] * len(rows_to_write))
        try:
            self.database.execute(ex_str % tuple(rows_to_write))
        except (sqlite3.OperationalError, sqlite3.SQLException), msg:
            if 'no such table' in msg.message:
                raise TableMissing
            if not('has no column named' in msg.message):
    ...
    def new_column(self, table_name, data_to_write):
        existing_columns = self.database.column_names(table_name)
        rows_to_write = data_to_write.values()
