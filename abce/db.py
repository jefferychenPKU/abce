...
try:
    ...
    if __name__ == '__main__':
        print("can not run db.py with jython")
        raise
    import subprocess
    import os
    class Database: #pylint: disable=R0903
        def __init__(self, simulation_path, db_name, addresses_bind):
            path = os.path.dirname(__file__)
            self.process = subprocess.Popen(['python', path + '/db.py', simulation_path, db_name, addresses_bind['database']])

        def is_alive(self):
            return self.process.poll() is None #pylint: disable=E1101
    ...
    class Database(multiprocessing.Process):
        def __init__(self, directory, db_name, address_bind):
            ...
            for t in (np.int8, np.int16, np.int32, np.int64, #pylint: disable=E1101
                                        np.uint8, np.uint16, np.uint32, np.uint64): #pylint: disable=E1101
                sqlite3.register_adapter(t, long)
            for t in (np.float, np.float16, np.float32, np.float64): #pylint: disable=E1101
                sqlite3.register_adapter(t, float)
            self._address_database = address_bind
        ...
        def run(self):
            ...
            in_sok.bind(self._address_database)
            ...
            while True:
                ...
                if typ == 'panel':
                    ...
                    data_to_write = in_sok.recv_pyobj()
                    data_to_write = dict([(key, float(data_to_write[key])) for key in data_to_write])
                    data_to_write['round'] = int(in_sok.recv())
    ...
    class TableMissing(sqlite3.OperationalError, Exception):
        pass
...
if __name__ == '__main__':
    import sys
    db = Database(directory=sys.argv[1], db_name=sys.argv[2], address_bind=sys.argv[3])
    db.start()
